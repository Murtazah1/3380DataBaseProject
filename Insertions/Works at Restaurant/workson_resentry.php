...
$sql = "INSERT INTO WORKSON_RES (EMPID, DEPID, RESNAME) VALUES (:empid, :depid, :resname)";
$stmt = oci_parse($conn, $sql);

// Bind parameters to statement
oci_bind_by_name($stmt, ":empid", $empid);
oci_bind_by_name($stmt, ":depid", $depid);
oci_bind_by_name($stmt, ":resname", $resname);
...
if ($result === true) {
    ...
    if ($num_rows == 0) {
        ...
        echo '<h1>Insert New Works On Restaurant Record</h1>';
        echo '</div>';
        ...
        echo "<p><strong>Restaurant Name:</strong> " . $resname . "</p>";
        echo '</div>';
    }
} 
else {
...
<html>
<head>
	<title>Insert New Works on Restaurant Record</title>
    <style>
        .insert-record {
            display:block;
